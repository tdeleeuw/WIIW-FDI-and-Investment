*****************************************************************
***               STATA code for construction of              ***
********          LEND and GVA reliance proxies          ********
*****************************************************************
******* BEFORE STARTING: *************
* 1. Set a WORKING DIRECTORY (folder) in line 14.
*    This folder should already exist on your computer.
*    The global defined in line 14 will ensure the code can access this folder on various code lines.
*    Type "help global" for details.
* 2. For the LEND measure data from a range of different central
*    banks is required: check readme file for sources.
*****************************************************************
...
// CZECHIA LENDING //
* In excel file, column titles (industry names) were changed to IND1, IND2, etc.
...
save "$path\czechialoan.dta", replace
clear
...
save "$path\hungaryloan.dta", replace
clear
...
save "$path\estonialoan.dta", replace
clear
...
save "$path\bulgarialoan.dta", replace
clear
...
merge 1:1 country industry year using "$path\albpriv.dta", nogenerate
...
drop CountryCode country lending yearsum
save "$path\albanialoan.dta", replace
clear
...
save "$path\slovakialoan.dta", replace
clear

// MERGING // 
use "$path\albanialoan.dta"

merge 1:1 year sector using "$path\hungaryloan.dta", nogenerate
merge 1:1 year sector using "$path\estonialoan.dta", nogenerate
merge 1:1 year sector using "$path\slovakialoan.dta", nogenerate
merge 1:1 year sector using "$path\czechialoan.dta", nogenerate
merge 1:1 year sector using "$path\bulgarialoan.dta", nogenerate
sort year sector
...
save "$path\lendcoeff1.dta", replace
clear



// GVA PROXY //
use "$path\gvapanel.dta"
